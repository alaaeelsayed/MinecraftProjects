...
public class ScubaUpgrade extends Upgrade {

	public ScubaUpgrade(int level) {
		super("scuba", level, 1, HeadEnchants.getConfigManager().getLore("scuba"), ItemType.HELMET);
	...
	public void onWear(ArmorEquipEvent event, Player player, boolean removed) {
		if (!removed) {
			player.addPotionEffect(new PotionEffect(PotionEffectType.WATER_BREATHING, Integer.MAX_VALUE, getLevel() - 1));
		} else {
